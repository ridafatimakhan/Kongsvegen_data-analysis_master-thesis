...
Description:
This script processes filtered sensor data (after initial cleaning) to analyze acceleration and pressure readings.
It performs the following tasks:

1. Loads time-series data including pressure and IMU readings (accelerometer, gyroscope, magnetometer).
2. Rotates the accelerometer data from the sensor (body) frame to the global frame.
3. Applies a Kalman filter to smooth the rotated acceleration signals.
4. Detects potential step-like movements based on vertical (Z-axis) acceleration thresholds.
5. Identifies high-pressure regions across two pressure sensors using a dual-sensor dynamic thresholding approach.
6. Visualizes the filtered acceleration and pressure data, highlighting detected steps and pressure pool regions.
7. Saves the generated plot as a PDF and prints timing summaries for detected events.

Note:
- Adjust file paths, thresholds, and parameters as needed for different datasets.
- Ensure required libraries (NumPy, Pandas, Matplotlib, Seaborn) are installed.
"""
...
file_path = 'H:/Rida/Kongsvegen_data/Screened_data/18072021/M04/M040718173701.txt/filtered_pressure1.txt'
column_names = [
...
# Parameters
Z_THRESHOLD = 5  #  (lower = more sensitive)
